...
- **Agent self-discovery**: run `dhcli agents tree` for the compact summary of the live tree (`--full` for the complete manifest with per-command output schema), or append `--agents` to any command for its full node. A node names the error codes it can emit but not their meanings — `dhcli agents errors` is the decoder (`ref-cli-help-standards` *What the agents manifest carries*).
