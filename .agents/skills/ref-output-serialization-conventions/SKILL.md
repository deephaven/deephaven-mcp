---
name: ref-output-serialization-conventions
...
**`programming_language` is title case (`"Python"` / `"Groovy"`), not lowercase.** Like `auth_type`, its vocabulary is defined externally by Deephaven: the enterprise controller's `scriptLanguage` field takes these exact strings. The single source is the `ProgrammingLanguage` Literal in `sessions/_types.py`; every schema field and tool parameter uses it. Known inconsistency: `session_details` echoes the live session's self-reported value (`_get_session_programming_language` in `_tools/session.py`), which is lowercase (`"python"`) — a coordinated fix would normalize that boundary to the canonical vocabulary. Do not lowercase the canonical values to match the categorical-label convention.
...
- **Shape matches the data.** A list of names is a plain array of strings, not a single-column table. Tabular envelopes (`columns` + `data` + `format`) are for genuinely multi-column data (`session_table_data`). Listing and schema tools return lean identity dicts instead (`catalog_tables_list`'s `{namespace, table_name}` entries; `session_table_schema`'s `{name, type}` column entries) so agent context is not flooded.
- **Sparse optional keys.** A key that is only meaningful for a minority of items is omitted when it does not apply — never emitted with a placeholder value (`null`, `false`, a default like `"Normal"`). Absence is the documented default: the tool docstring and CLI `OutputSpec` state the omission rule. Rationale: the consumers are agents reading JSON — a rare key is salient, a per-item placeholder is context noise. Canonical implementations: the envelope's `isError` (present and True only on error responses); `format_schema_result` in `mcp_systems_server/_tools/shared.py` (`column_type` omitted for Normal columns). Known inconsistency: `mcp_systems_server/_tools/pq.py` predates this rule — `_format_pq_config` renders a full-detail PQ config as a fixed shape with `None` placeholders for unset optional fields (only `worker_kind` is sparse so far), and the batch tools initialize per-item result keys to `None` (`name` / `state` / `state_category` / `error` in `pq_start` / `pq_stop` / `pq_restart`; `name` / `error` in `pq_delete`). Both await a coordinated migration; do not copy the placeholder pattern into new payloads.
...
- **The echo-back rule governs the MCP layer, not the CLI projection.** Its rationale is detachment: a tool payload outlives its request. A CLI invocation does not — argv carries the attribution, visible above the output for a human and constructed by the agent that reads the response — so a shaping wrapper (`call_and_echo_field`) legitimately drops the echoed `id`/`system`, and `session_tables_list` / `catalog_tables_list` / `catalog_namespaces_list` / `session_pip_list` / `pq_list` all echo correctly even though `dhcli table list` prints a bare array. Do **not** "fix" that by wrapping the array: the bare shape is the contract that `-o human`'s table renderer and `jq '.[]'` consume. The one case argv does not cover — an argument filled from the sticky context — is closed in help text (`ref-cli-help-standards` _Help-content contract_) and, opt-in, by a confirmation prompt (`ref-cli-tool-wrapping` _Destructive verbs with a defaultable target_).
...
- **Bounded output.** A tool that can return unbounded data takes a `max_rows` cap with a conservative default (`max_rows=None` is an explicit caller opt-in to an uncapped query, never the default) and guards a serialized-size estimate of the response against the configured `ResponseLimits` before assembling the payload (`get_response_limits` / `check_response_size` in `mcp_systems_server/_tools/shared.py`); when the emitted form repeats key names per entry, the estimate adds a per-entry overhead allowance on top of raw buffer bytes (canonical: `_ENTRY_OVERHEAD_BYTES` in `mcp_systems_server/_tools/catalog.py`), and the over-limit error names the remedy (reduce `max_rows`). The path to more data is a narrower query, not a lifted cap.
...
- **Exceptions embedded in `error` fields render via `deephaven_mcp._exception_utils`** — apply `ref-python-coding-practices` rule 20, which owns the canonical form, the renderer choice, and the parsed-contract rationale.
