...
description: Verify dhcli CLI help is factually accurate against the code — invoke for surgical correctness-only fixes when a command's help structure is already complete; use cli-help-improve for a full review. Checks that documented flags, arguments, error codes, exit codes, and output fields match the handler, and fixes inaccuracies across --help, the agents manifest, and docs/CLI.md
---
...
Load `ref-cli-help-standards`. `--help` and the agents manifest both render from the command's `HelpSpec`, so fixing the spec fixes both; `docs/CLI.md` is authored separately and must be reconciled by hand. Fixing an inaccuracy in the spec without the doc (or vice versa) reintroduces drift, so check and fix both together.
