...
Helper failure contract: any `_tools` helper that can fail raises a named exception, which the calling tool's `except` handler converts to the error dict. Never mirror a failure through a `(value, error)` tuple and `cast()` past the `None` — `cast` is unchecked, so mypy cannot catch a broken invariant. Canonical implementations: `get_enterprise_session` in `_tools/shared.py`; `_setup_batch_pq_operation` in `_tools/pq.py` (raises, returns a frozen dataclass with non-optional fields).
