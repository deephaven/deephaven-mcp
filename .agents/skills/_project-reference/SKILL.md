...
- `src/deephaven_mcp/cli/` — Local `dh-mcp` CLI (click + Pattern B). `_main.py` (root group), `_commands/{daemon,tool,session,system,table,catalog,pq,docs,config,agents}.py` (noun groups; `daemon` for daemon lifecycle, `tool` is the raw MCP escape hatch, `config` reads the local config tree, the runtime nouns `session`/`system`/`table`/`catalog`/`pq` wrap specific MCP tools, `docs` connects directly to the docs MCP server (no daemon), `agents` emits machine-readable CLI metadata), `_async.py` (`run_async` async-to-sync adapter), `_errors.py` (`CliError` + `ErrorCode` registry), `_help.py` (`HelpSpec` help templating + the agents manifest builders), `_format.py` (human/json/json-pretty/yaml renderers), `_runtime.py` (resolved `Runtime` context on `ctx.obj`), `_daemon/` (daemon-lifecycle package: `_lifecycle.py` orchestration core `get_or_start_daemon(ctx, ...) -> DaemonRegistryEntry` / `stop_daemon(directory, *, kill_after_seconds)`; commands build the `DaemonContext` from a `Runtime` via `DaemonContext.from_runtime(runtime)` and read tunables from `runtime.config.cli.daemon`; the OS-specific spawn mechanic is delegated to `_platform.spawn.spawn_detached`), `_mcp_client.py` (loopback HTTP client). Async handlers must be wrapped with `@run_async` — see `_python-coding-practices` rule 15 and the `cli-command-add` skill.
...
Thin local client for the systems server: auto-spawns a per-user background daemon on first use and dispatches MCP tool calls over loopback HTTP with a PSK. Built on `click` (>=8.4) with Pattern B async wrapping (`@run_async` from `cli/_async.py`). Noun-verb command tree under `cli/_commands/{daemon,tool,session,system,table,catalog,pq,docs,config,agents}.py`.

- **Reference**: `docs/CLI.md` is the full command/flag/exit-code/error-code surface.
- **Agent self-discovery**: run `dh-mcp agents tree` for the compact summary of the live tree (`--full` for the complete manifest with per-command output schema), or append `--agents` to any command for its self-contained node.
- **Machine-first defaults**: every command (including the agents surfaces) defaults to `json` — compact single-line JSON; opt into indented JSON with `-o json-pretty` or terminal-friendly output with `-o human` / `DH_MCP_OUTPUT=human`. The agents surfaces honor `-o`/`DH_MCP_OUTPUT` only — they run before the runtime config load and do not read `cli.json`'s `output.format`.
- **Surface contract**: `--help` and the agents manifest render from each command's `HelpSpec`; `docs/CLI.md` must be reconciled with the spec by hand. The help-content contract is `_cli-help-standards`.
...
cli.json                       # dh-mcp CLI settings (output format, daemon/request tunables)
community/
