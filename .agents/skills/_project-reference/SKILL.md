...
- `src/deephaven_mcp/mcp_systems_server/config/` — Systems-server-specific Pydantic schemas and orchestration: `_tree.py` (walks the configuration directory and produces `ConfigTree` via `ConfigTreeLoader`) plus the per-section schema/loader modules `_server.py`, `_community.py`, `_enterprise.py` (each owns its umbrella schema and `load_<section>` function). `ConfigTree` mirrors the on-disk layout (`server.json`, `cli.json`, `community/`, `enterprise/`) one-for-one and is the canonical aggregator type.
