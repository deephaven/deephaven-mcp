...
- **List only the surface submodules** — the ones imported from outside the package. Omit underscored/internal submodules (`_os_support.py`, `_pq_config.py`): they are the package-private analogue of a private helper and do not belong in the documented surface. Canonical implementation: `mcp_systems_server/_tools/__init__.py` (lists `session`/`table`/… and omits `_pq_config.py`/`_response_limits.py`); `auth/__init__.py` (lean `:mod:` bullet list, no rationale).
