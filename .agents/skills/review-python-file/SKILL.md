...
5. **Security**: Check for credential mishandling, session isolation issues, injection risks, and information disclosure — or anything else security-relevant. This list is illustrative, not exhaustive; trust your judgement. Flag any default or fallback ids — fully qualified ids arrive as explicit tool parameters and are validated by `QualifiedSessionId.from_str`, which raises rather than substituting a default.
