...
- **Exceptions embedded in `error` fields render in the canonical `TypeName: message` form** via `deephaven_mcp._exception_utils` (`exception_summary` for one exception, `describe_exception` for a failure tree) — never a hand-typed `f"{type(e).__name__}: {e}"`. The form is a parsed contract, not a style choice: `_short_reason` (`_tools/session_enterprise.py`) extracts the type name back out of recorded init errors to build the `liveness_detail` column. See `_python-coding-practices` rule 20.
