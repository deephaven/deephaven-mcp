...
@click.group(
    ...
    help_spec=HelpSpec(
        ...
        description=(
            ...
            "work in a session you were given or one you make yourself "
            "with 'dhcli session create dev' (the daemon auto-starts on "
            "first use), then use a verb like 'dhcli table data' to read "
            "a table or 'dhcli session exec' to run a script. 'dhcli "
            "session list' and 'dhcli pq list' are for discovery, not "
            "for picking a target: they span every user's resources, "
            "production included, so never run scripts in, stop, or "
            "delete an entry you did not create and were not pointed at. "
            "'session create' records the new id as the default target, "
            "or set one with 'dhcli context set session ID', and later "
            "commands can omit the id — run 'dhcli context show' "
