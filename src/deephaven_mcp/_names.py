...
- a dot-separated configuration path segment,
- a shell argument.

The rule is the same in every case (ASCII alphanumerics plus ``_`` and
``-``, starting with an alphanumeric, non-empty), so it lives in one
place. Dots are excluded so a name can appear as one segment of a
dot-separated configuration path (``community.sessions.<name>.port``)
without ambiguity.
...
Allowed: ASCII alphanumerics plus ``_`` and ``-``. First character must be
alphanumeric (no leading dash or underscore); dots are rejected everywhere
so the name can serve as one segment of a dot-separated configuration
path. Empty strings are rejected. Display names (e.g. enterprise PQ names
from the DHE controller) are NOT subject to this rule; they travel through
verbatim.
"""
...
def validate_resource_name(value: str, *, field: str) -> str:
    ...
    ``[A-Za-z0-9_-]`` and starting with an alphanumeric character. Dots
    are rejected so the name can appear as one segment of a
    ...
    Raises:
        InvalidSessionNameError: When ``value`` is empty, contains any
            character outside ``[A-Za-z0-9_-]`` (including ``.``), or does
            not start with an alphanumeric (a leading ``-`` or ``_`` is
            rejected).
    """
    if not value:
    ...
    if not _RESOURCE_NAME_PATTERN.match(value):
        ...
        if bad_chars:
            ...
            if "." in bad_chars:
                detail += " (dots are not allowed in names; rename using '-' or '_')"
        else:
