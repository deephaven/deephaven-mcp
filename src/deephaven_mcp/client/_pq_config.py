...
# Generated by the vendor helper so the entry set stays in parity with the vendor client
# rather than drifting as a hand-rolled copy.
_DEFAULT_PERMANENT_CONTINUOUS_SCHEDULING: tuple[str, ...] = tuple(
...
def _set_termination_delay(
    ...
    if fields:
        config_pb.typeSpecificFieldsJson = json.dumps(fields)
    else:
        # typeSpecificFieldsJson is a presence-tracked optional field; assigning ""
        # marks it present-but-empty, which the controller's deserializer rejects
        # with ControllerSerializationException. Clear it instead.
        config_pb.ClearField("typeSpecificFieldsJson")
