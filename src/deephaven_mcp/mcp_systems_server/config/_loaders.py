"""Shared loader helpers for the systems-server config tree.

Extracts the recurring "load JSON file -> validate against Pydantic
model -> redact-log" pipeline used by every per-section loader in
:mod:`deephaven_mcp.mcp_systems_server.config`. Each public helper
takes the model class, the source file path, and the audited
configuration root; it returns a validated model instance or raises
...
- :func:`load_named_json` — loads a single configuration file whose
  contents map directly onto the model fields (e.g. ``server.json``,
  ``community/settings.json``, ``enterprise/settings.json``).
