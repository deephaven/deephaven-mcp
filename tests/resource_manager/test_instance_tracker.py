...
def temp_instances_dir(tmp_path):
    ...
    with patch("deephaven_mcp.resource_manager._instance_tracker.Path.home", return_value=tmp_path):
        yield instances_dir
...
class TestCleanupOrphanedResources:
    ...
    async def test_cleanup_dead_instance_no_resources(self, temp_instances_dir):
        ...
        # Mock Docker cleanup
        with patch("deephaven_mcp.resource_manager._instance_tracker._cleanup_docker_containers_for_instance") as mock_docker:
    ...
    async def test_cleanup_dead_instance_with_pip_processes(self, temp_instances_dir):
        ...
        # Mock Docker cleanup
        with patch("deephaven_mcp.resource_manager._instance_tracker._cleanup_docker_containers_for_instance") as mock_docker, \
             patch("deephaven_mcp.resource_manager._instance_tracker.is_process_running") as mock_is_running, \
             patch("os.kill") as mock_kill:
            
            mock_docker.return_value = AsyncMock()
            
            # First call checks the instance PID (dead), subsequent calls check pip processes
            mock_is_running.side_effect = [False, True, True]  # Instance dead, both pip processes alive
    ...
    async def test_cleanup_handles_errors_gracefully(self, temp_instances_dir):
        ...
        with patch("deephaven_mcp.resource_manager._instance_tracker._cleanup_docker_containers_for_instance", side_effect=docker_cleanup_side_effect):
    ...
    async def test_cleanup_docker_ps_failure(self, temp_instances_dir):
        ...
        mock_ps.communicate = AsyncMock(return_value=(b"", b"Docker daemon not running"))
    ...
    async def test_cleanup_no_docker_containers_found(self, temp_instances_dir):
        ...
        with patch("deephaven_mcp.resource_manager._instance_tracker.is_process_running", return_value=False):
    ...
    async def test_cleanup_docker_stop_error(self, temp_instances_dir):
        ...
        with patch("deephaven_mcp.resource_manager._instance_tracker.is_process_running", return_value=False):
            ...
            mock_stop.communicate = AsyncMock(return_value=(b"", b"Container not found"))
            ...
            async def mock_exec(*args, **kwargs):
                if args[0] == "docker" and args[1] == "ps":
                    ...
                    return mock_stop
                return AsyncMock(returncode=0, communicate=AsyncMock(return_value=(b"", b"")))
    ...
    async def test_cleanup_pip_process_kill_error(self, temp_instances_dir):
        ...
        with patch("deephaven_mcp.resource_manager._instance_tracker.is_process_running", side_effect=mock_is_running):
            # Mock no Docker containers
            mock_ps = AsyncMock()
            mock_ps.returncode = 0
            mock_ps.communicate = AsyncMock(return_value=(b"", b""))
