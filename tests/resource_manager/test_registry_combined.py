...
def initialized_registry(combined_registry, mock_config_manager):
    ...
    combined_registry._enterprise_registry = MagicMock(spec=CorePlusSessionFactoryRegistry)
...
class TestCombinedSessionRegistryInitialization:
    ...
    async def test_initialize_success(self, combined_registry, mock_config_manager):
        ...
        with patch.object(
            CommunitySessionRegistry, "__new__", return_value=MagicMock(spec=CommunitySessionRegistry)
        ) as mock_community_cls, patch.object(
            CorePlusSessionFactoryRegistry, "__new__", return_value=MagicMock(spec=CorePlusSessionFactoryRegistry)
        ) as mock_enterprise_cls, patch.object(
            ...
            mock_community_registry.initialize.assert_called_once_with(mock_config_manager)
            mock_enterprise_registry.initialize.assert_called_once_with(mock_config_manager)
    ...
    @pytest.mark.asyncio
    async def test_initialize_community_failure(self, combined_registry, mock_config_manager):
        """Test handling of community registry initialization failure."""
        # Patch the registries and update_enterprise_sessions
        combined_registry._update_enterprise_sessions = AsyncMock()
        mock_update = combined_registry._update_enterprise_sessions
        
        with patch('deephaven_mcp.resource_manager._registry_combined.CommunitySessionRegistry') as mock_community_cls, \
             patch('deephaven_mcp.resource_manager._registry_combined.CorePlusSessionFactoryRegistry') as mock_enterprise_cls:
            
            # Set up community registry to fail initialization
            mock_community_registry = mock_community_cls.return_value
            mock_community_registry.initialize = AsyncMock(side_effect=Exception("Community initialization failed"))
            ...
            mock_community_registry.initialize.assert_awaited_once_with(mock_config_manager)
    ...
    @pytest.mark.asyncio
    async def test_initialize_enterprise_failure(self, combined_registry, mock_config_manager):
        """Test handling of enterprise registry initialization failure."""
        # Patch the registries and update_enterprise_sessions
        with patch.object(combined_registry, '_community_registry', None), \
             patch.object(combined_registry, '_enterprise_registry', None), \
             patch.object(combined_registry, '_update_enterprise_sessions', AsyncMock()) as mock_update, \
             patch('deephaven_mcp.resource_manager._registry_combined.CommunitySessionRegistry') as mock_community_cls, \
             patch('deephaven_mcp.resource_manager._registry_combined.CorePlusSessionFactoryRegistry') as mock_enterprise_cls:
            ...
            mock_enterprise_registry = mock_enterprise_cls.return_value
            mock_enterprise_registry.initialize = AsyncMock(side_effect=Exception("Enterprise initialization failed"))
            ...
            mock_community_registry.initialize.assert_awaited_once_with(mock_config_manager)
            mock_enterprise_registry.initialize.assert_awaited_once_with(mock_config_manager)
    ...
    @pytest.mark.asyncio
    async def test_double_initialization_warning(self, combined_registry, mock_config_manager):
        ...
        with patch.object(combined_registry, "_update_enterprise_sessions", AsyncMock()) as mock_update:
            with patch("deephaven_mcp.resource_manager._registry_combined._LOGGER.warning") as mock_warn:
                await combined_registry.initialize(mock_config_manager)
...
class TestControllerClientCaching:
    ...
    async def test_get_or_create_controller_client_new(self, combined_registry):
        ...
        mock_factory.create_controller_client = AsyncMock(return_value=mock_client)
        mock_client.subscribe = AsyncMock()
        
        # Call the method
        client = await combined_registry._get_or_create_controller_client(mock_factory, "test_factory")
        
        # Verify client was created, subscribed, and cached
        mock_factory.create_controller_client.assert_awaited_once()
        mock_client.subscribe.assert_awaited_once()
        assert combined_registry._controller_clients["test_factory"] == mock_client
        assert client == mock_client
    
    @pytest.mark.asyncio
    async def test_get_or_create_controller_client_reuse_healthy(self, combined_registry):
        ...
        with patch('deephaven_mcp.resource_manager._registry_combined.CorePlusControllerClient',
                   MagicMock()) as mock_controller_client_cls:
            # Mock client creation for the factory
            mock_controller_client_cls.return_value = MagicMock(spec=CorePlusControllerClient)
            
            # Set up the factory
            factory = MagicMock()
            
            # Call should reuse the client since it's healthy
            client = await combined_registry._get_or_create_controller_client(factory, "test_factory")
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_controller_client_recreate_dead(self, combined_registry):
        """Test recreating a controller client when the existing one is dead."""
        # Setup mocks
        mock_factory = MagicMock(spec=CorePlusSessionFactoryManager)
        ...
        mock_factory.create_controller_client = AsyncMock(return_value=mock_new_client)
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_controller_client_close_exception(self, combined_registry):
        ...
        # Mock failed health check (False result) and successful recreation
        mock_old_client.ping = AsyncMock(return_value=False)
        mock_old_client.close = AsyncMock(side_effect=RuntimeError("Failed to close client"))
...
class TestEnterpriseSessionUpdate:
    ...
    async def test_update_enterprise_sessions_success(self, initialized_registry):
        ...
        with patch.object(
            initialized_registry, "_get_or_create_controller_client", AsyncMock(return_value=mock_client)
        ):
            # Mock session map from controller
            mock_client.map = AsyncMock(return_value={"session1": {"id": "123"}})
            mock_client.ping = AsyncMock(return_value=True)
    
            # Mock session creation
            with patch.object(
                CombinedSessionRegistry, "_make_enterprise_session_manager", return_value=mock_session
            ) as mock_make_session:
                # Update sessions
                await initialized_registry._update_enterprise_sessions()
    
                # Verify session was created and added
                mock_make_session.assert_called_once_with(mock_factory, "factory1", "session1")
    
                # Verify session was added to items using the session's full_name property
                assert initialized_registry._items[mock_session.full_name] == mock_session
                
    @pytest.mark.asyncio
    async def test_update_enterprise_sessions_reuses_existing(self, initialized_registry):
        ...
        with patch.object(initialized_registry, "_make_enterprise_session_manager", 
                          wraps=initialized_registry._make_enterprise_session_manager) as make_spy:
            ...
            for call in make_spy.mock_calls:
                ...
                if len(call[1]) >= 3 and call[1][2] == "session1":
                    assert False, "_make_enterprise_session_manager was called for session1 when it should be reused"
    ...
    @pytest.mark.asyncio
    async def test_close_stale_sessions_ignores_nonexistent_key(self, initialized_registry):
    ...
    async def test_update_enterprise_sessions_removes_stale(self, initialized_registry):
        ...
        stale_key = BaseItemManager.make_full_name(SystemType.ENTERPRISE, "factory1", "old_session")
...
def test_add_new_enterprise_sessions_skips_existing(initialized_registry):
    ...
    with patch.object(
        ...
        # Act
        initialized_registry._add_new_enterprise_sessions(factory, factory_name, sessions)
...
class TestClose:
    ...
    async def test_close_handles_exceptions(self, initialized_registry):
        ...
        # Setup mocks
        mock_community_registry = initialized_registry._community_registry
        mock_community_registry.close = AsyncMock(side_effect=Exception("Community close failed"))
        
        mock_enterprise_registry = initialized_registry._enterprise_registry
        mock_enterprise_registry.close = AsyncMock(side_effect=Exception("Enterprise close failed"))
        ...
        with patch("deephaven_mcp.resource_manager._registry_combined._LOGGER.error") as mock_error:
            await initialized_registry.close()
...
class TestMakeEnterpriseSessionManager:
    ...
    def test_make_enterprise_session_manager(self):
        ...
        # Setup mocks
        mock_factory = MagicMock(spec=CorePlusSessionFactoryManager)
        mock_factory.connect_to_persistent_query = AsyncMock()
        ...
        mock_factory.connect_to_persistent_query.assert_awaited_once_with("session1")
