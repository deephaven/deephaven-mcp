...
class ConcreteItemManager(BaseItemManager[MockItem]):
    ...
    async def _check_liveness(self, item: MockItem) -> tuple[ResourceLivenessStatus, str | None]:
        try:
...
async def test_close_handles_sync_method_gracefully():
    ...
    class ConcreteSyncItemManager(BaseItemManager[MockSyncItem]):
        ...
        async def _check_liveness(self, item: MockSyncItem) -> tuple[ResourceLivenessStatus, str | None]:
...
async def test_liveness_status_unlocked_exceptions(monkeypatch):
    ...
    monkeypatch.setattr(manager, "_get_unlocked", AsyncMock(side_effect=AuthenticationError("authfail")))
    ...
    monkeypatch.setattr(manager, "_get_unlocked", AsyncMock(side_effect=ConfigurationError("cfgfail")))
    result = await manager._liveness_status_unlocked(ensure_item=True)
    ...
    monkeypatch.setattr(manager, "_get_unlocked", AsyncMock(side_effect=SessionCreationError("scfail")))
    result = await manager._liveness_status_unlocked(ensure_item=True)
    assert result[0] == ResourceLivenessStatus.MISCONFIGURED
    assert "scfail" in result[1]

    # Patch _get_unlocked to raise generic Exception
    monkeypatch.setattr(manager, "_get_unlocked", AsyncMock(side_effect=RuntimeError("boom!")))
    result = await manager._liveness_status_unlocked(ensure_item=True)
...
async def test_close_logs_on_liveness_failure(monkeypatch, caplog):
    ...
    monkeypatch.setattr(manager, "_is_alive_unlocked", AsyncMock(side_effect=Exception("liveness fail!")))
    with caplog.at_level("INFO"):
        await manager.close()
    expected = "[DummyManager] Successfully closed item for 'community:src:nm' despite earlier liveness failure"
    assert any(r.levelname == "INFO" and r.getMessage() == expected for r in caplog.records)
...
class TestCorePlusSessionFactoryManager:
    ...
    async def test_check_liveness(self):
        ...
        assert await manager._check_liveness(mock_factory) == (ResourceLivenessStatus.ONLINE, None)
        ...
        assert await manager._check_liveness(mock_factory) == (ResourceLivenessStatus.OFFLINE, "Ping returned False")
        mock_factory.ping.assert_awaited_once()
