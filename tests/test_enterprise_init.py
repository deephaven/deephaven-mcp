"""Tests for the deephaven_mcp.enterprise package init."""
...
def test_enterprise_init_coreplus_client_not_found():
    ...
    with mock.patch('importlib.util.find_spec', side_effect=mock_find_spec):
        with pytest.raises(ImportError, match="The 'deephaven_mcp.enterprise' module requires 'deephaven-coreplus-client' to be installed"):
...
def test_enterprise_init_coreplus_client_found(capfd):
    ...
    with mock.patch.dict(importlib.sys.modules, {'deephaven_coreplus_client': mock_coreplus}):
        try:
            imported_module = importlib.import_module(MODULE_UNDER_TEST)
            assert imported_module is not None
        except ImportError as e:
            pytest.fail(f"Importing {MODULE_UNDER_TEST} failed when deephaven_coreplus_client was mocked: {e}")

        # Check for the print statements
        captured = capfd.readouterr()
        assert "INFO: 'deephaven-coreplus-client' found. Deephaven MCP enterprise submodule is available." in captured.out
