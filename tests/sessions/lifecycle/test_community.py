...
async def test__create_session_impl_success():
    ...
    with patch("deephaven_mcp.sessions._lifecycle.community.Session", return_value=DummySession()) as mock_sess:
        result = await _lifecycle_community._create_session_impl(host="localhost", port=10000)
...
async def test__create_session_impl_failure():
    ...
    with patch("deephaven_mcp.sessions._lifecycle.community.Session", side_effect=RuntimeError("fail!")):
        with pytest.raises(SessionCreationError) as excinfo:
            await _lifecycle_community._create_session_impl(host="localhost", port=10000)
...
async def test_create_session_for_worker_session_fail(monkeypatch):
    ...
    monkeypatch.setattr(_lifecycle_community, "_create_session_impl", fake_create_session_impl)
    with pytest.raises(RuntimeError):
        await _lifecycle_community.create_session_for_worker(cfg_mgr, "workerZ")
