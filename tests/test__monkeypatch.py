...
def test_is_client_disconnect_error():
    ...
    group = ExceptionGroup("g", [closed_error, value_error])
    assert monkeypatch_mod._is_client_disconnect_error(group) is True

    # Test exception group without ClosedResourceError
    group_no_closed = ExceptionGroup("g", [value_error, RuntimeError("test")])
    assert monkeypatch_mod._is_client_disconnect_error(group_no_closed) is False
