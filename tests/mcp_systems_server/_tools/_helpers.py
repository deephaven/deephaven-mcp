"""Shared test helpers for mcp_systems_server tool tests.
...
Imported explicitly by sibling modules (``from ._helpers import
MockContext``), matching ``tests/cli/_helpers.py``. These are plain
helpers rather than pytest fixtures, so they do not belong in a
``conftest.py``.
"""
