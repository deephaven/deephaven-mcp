"""End-to-end tests for :class:`deephaven_mcp.config._multi.MultiSystemConfigManager`.

These tests drive the manager against real on-disk fixture trees and
exercise the cooperation between :mod:`_multi`, :mod:`_config_dir`,
...
from deephaven_mcp.config._config_dir import CONFIG_DIR_ENV_VAR, default_config_dir
from deephaven_mcp.mcp_systems_server.config._multi import MultiSystemConfigManager
...
def _make_manager(config_dir: Path) -> MultiSystemConfigManager:
    """Lock down ``config_dir`` and return an *uninitialized* manager."""
    _lock_down(config_dir)
    return MultiSystemConfigManager(config_dir)


async def _load(config_dir: Path) -> MultiSystemConfigManager:
...
def test_manager_config_dir_explicit_wins(tmp_path: Path) -> None:
    ...
    with patch.dict(os.environ, {CONFIG_DIR_ENV_VAR: str(tmp_path / "env")}):
        assert MultiSystemConfigManager(explicit).config_dir == explicit


def test_manager_config_dir_env_var(tmp_path: Path) -> None:
    env_value = str(tmp_path / "from_env")
    with patch.dict(os.environ, {CONFIG_DIR_ENV_VAR: env_value}):
        assert MultiSystemConfigManager().config_dir == Path(env_value)
...
def test_manager_config_dir_default_when_env_unset() -> None:
    ...
    with patch.dict(os.environ, env, clear=True):
        assert MultiSystemConfigManager().config_dir == default_config_dir()


# ---------------------------------------------------------------------------
# MultiSystemConfigManager: empty / required-system handling
# ---------------------------------------------------------------------------
...
async def test_missing_dir_fails(tmp_path: Path) -> None:
    manager = MultiSystemConfigManager(tmp_path / "nope")
...
async def test_examples_ai_config_loads_end_to_end(
    ...
    :class:`MultiSystemConfigManager` the production lifespan uses.
    """
    import shutil
