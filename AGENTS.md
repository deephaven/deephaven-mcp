...
- **Structure**: noun-verb groups under `src/deephaven_mcp/cli/_commands/`. Top-level nouns: `daemon` (process lifecycle), `tool` (raw MCP escape hatch), `session`/`system`/`table`/`catalog`/`pq` (runtime MCP-tool wrappers), `config` (static config), `introspect` (machine-readable metadata: `tree`/`command`/`errors` verbs). Plus a universal `--introspect` flag on every command — the machine-readable twin of `--help`.
