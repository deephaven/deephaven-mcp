...
- **Framework**: `click` (>=8.4); command callbacks that perform I/O are async (Pattern B async-to-sync adapter); pure-metadata verbs (the `agents` verbs, `self completion`) are plain synchronous functions. Apply `_python-coding-practices` rule 15 (`@run_async` from `cli/_async.py`, no `argparse`, `CliError` discipline) and the `cli-command-add` skill before adding or editing a command.
