...
- **Structure**: noun-verb groups under `src/deephaven_mcp/cli/_commands/`. Top-level nouns: `daemon` (process lifecycle), `tool` (raw MCP escape hatch), `session`/`system`/`table`/`script`/`catalog`/`pq` (runtime MCP-tool wrappers), `config` (static config). Meta command: `introspect`.
