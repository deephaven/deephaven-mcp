...
- **Configuration directory**: `dh-mcp-systems-server` reads a directory tree (not a single file). Override via `DH_MCP_CONFIG_DIR` env var or `--config-dir` CLI arg. Default: `~/.deephaven/ai/config/` (POSIX) or `%APPDATA%/Deephaven/ai/config/` (Windows). See `config-samples/ai/config/` for the layout.
