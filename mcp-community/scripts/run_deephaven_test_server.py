...
parser = argparse.ArgumentParser(
    description="Launch a Deephaven server with configurable host and port and table group."
)
parser.add_argument(
    "--host",
    type=str,
    default="localhost",
    help="Hostname or IP address to bind the server (default: localhost)",
)
parser.add_argument(
    "--port",
    type=int,
    default=10000,
    help="Port number for the Deephaven server (default: 10000)",
)
parser.add_argument(
    "--table-group",
    type=str,
...
if table_group == "simple" or table_group == "all":
    ...
    t3 = time_table("PT15m").update(
        ["C1 = ii%2 == 0 ? `abc` : `def`", "C2 = i", "C3 = sqrt(C2)"]
    )
...
if table_group == "financial" or table_group == "all":
    ...
    ohlcv = trades.update(["Bar = lowerBin(Timestamp, 'PT10s')"]).agg_by(
        [
