...
2. There should be a one-to-one correspondence between source files and test files.  Unless there is a strongly compelling reason, all tests for a python source file should be in a single test file.
3. All async functions registered via `server.tool()(fn)` inside a `register_tools(server: FastMCP)` function in a `_tools/` module are MCP tools.  Their associated pydocs will be consumed by AI agents.  As such, the pydocs need to be very clear and provide enough details so that the AI agent knows exactly how to use the tool and how exactly to interpret the results.
4. f-strings are preferred over % in format statements
5. All MCP tools in src/deephaven_mcp/mcp_systems_server/ or a subdirectory registered as MCP tools (via `server.tool()(fn)` in `register_tools`) must include a "Terminology Note" section with this exact wording:
