...
3. Call session_details with id="community:community:nonexistent-xyz" and attempt_to_connect=false. Verify success==false (this is expected — it is a negative test).
...
Pre-cleanup (idempotent): Call session_community_delete with id="community:community:mcp-test-create-delete". Ignore any error — this just ensures a clean starting state.

1. Call session_community_create with session_name="mcp-test-create-delete", launch_method="python", auth_type="anonymous". Verify success==true. Record the id.

2. Call sessions_list. Verify success==true. Verify "community:community:mcp-test-create-delete" appears in the list.

3. Call session_details with id="community:community:mcp-test-create-delete" and attempt_to_connect=true. Verify success==true.

4. CLEANUP: Call session_community_delete with id="community:community:mcp-test-create-delete". Verify success==true.
...
Pre-cleanup (idempotent): Call session_community_delete with id="community:community:mcp-test-script". Ignore any error — this just ensures a clean starting state.
...
1. Call session_script_run with id="community:community:mcp-test-script" and this script:
   from deephaven import new_table
   from deephaven.column import int_col, string_col
   hello_table = new_table([int_col("ID", [1, 2, 3]), string_col("Msg", ["hello", "world", "test"])])
   Verify success==true.

2. Call session_script_run with id="community:community:mcp-test-script" and script="this is not valid python !!!". Verify success==false (expected error).

3. Call session_script_run with id="community:community:mcp-test-script" with neither script nor script_path provided. Verify success==false and error mentions "Must provide either script or script_path".

CLEANUP: Call session_community_delete with id="community:community:mcp-test-script". Verify success==true.
...
Pre-cleanup (idempotent): Call session_community_delete with id="community:community:mcp-test-data". Ignore any error — this just ensures a clean starting state.
...
- Call session_script_run with id="community:community:mcp-test-data" and this script:
  from deephaven import new_table
  from deephaven.column import int_col, string_col, double_col
...
1. Call session_table_data with id="community:community:mcp-test-data" and table_name="results". Verify success==true. Verify 5 rows are returned with correct ID, Name, and Score values.

2. Call session_table_data with id="community:community:mcp-test-data", table_name="results", and max_rows=2. Verify success==true. Verify only 2 rows are returned.

3. Call session_table_data with id="community:community:mcp-test-data" and table_name="no_such_table_xyz". Verify success==false (expected error).

CLEANUP: Call session_community_delete with id="community:community:mcp-test-data". Verify success==true.
...
Pre-cleanup (idempotent): Call session_community_delete with id="community:community:mcp-test-pip". Ignore any error — this just ensures a clean starting state.
...
CLEANUP: Call session_community_delete with id="community:community:mcp-test-pip". Verify success==true.
...
Pre-cleanup (idempotent): Call session_community_delete with id="community:community:mcp-test-credentials". Ignore any error — this just ensures a clean starting state.
...
1. Call session_community_credentials with id="community:community:mcp-test-credentials".
...
2. Call session_community_credentials with id="community:community:nonexistent-xyz". Verify success==false (negative test — session does not exist).

3. Call session_community_credentials with id="enterprise:prod:some-session". Verify success==false and error mentions this tool only works for community sessions (negative test — wrong session type).

CLEANUP: Call session_community_delete with id="community:community:mcp-test-credentials". Verify success==true.
...
3. Call session_details with id="enterprise:nonexistent-system-xyz:fake-session" and attempt_to_connect=false. Verify success==false (negative test — system name does not exist on this server).
...
Pre-cleanup (idempotent): Call sessions_list. If a session named "mcp-test-ent-create-delete" appears in the list, note its id and call session_enterprise_delete with that id. Ignore any error.

1. Call session_enterprise_create with session_name="mcp-test-ent-create-delete". Verify success==true. Record the returned id (format: "enterprise:{system_name}:mcp-test-ent-create-delete") and system_name.

2. Call sessions_list. Verify success==true. Verify the id from step 1 appears in the list.

3. Call session_details with the id from step 1 and attempt_to_connect=true. Verify success==true.
...
5. CLEANUP: Call session_enterprise_delete with the id from step 1. Verify success==true.

6. Call sessions_list again. Verify the id from step 1 is no longer present.

7. Call session_enterprise_delete with the id from step 1 again (already deleted). Verify success==false (negative test — deleting a nonexistent session must be rejected).

Report: PASS/FAIL for each numbered step with a one-line explanation. Include the id from step 1.
```
...
Setup: Call session_enterprise_create with session_name="mcp-test-ent-shared". Verify success==true. Record the id.

1. Call session_script_run with the id from Setup and this script:
   from deephaven import new_table
   from deephaven.column import int_col, string_col
...
2. Call pq_details with id="enterprise:nonexistent-system-xyz:999999". Verify success==false (negative test — invalid id format or nonexistent system).

3. (Skip if step 1 returned zero PQs.) From the pq_list result, pick the first PQ. Record its id (format: "enterprise:{system_name}:{serial}") and name. Call pq_details with that id. Verify success==true. Verify the response includes a 'config' dict with at minimum 'name', 'serial', and 'script_language' fields.

4. (Skip if step 1 returned zero PQs.) Using the name recorded in step 3, call pq_name_to_id with pq_name set to that name. Verify success==true. Verify the returned id matches the one from step 3.
...
Pre-cleanup (idempotent): Call pq_list. If any PQ named "mcp-test-pq-lifecycle" is found, record its id and call pq_delete with that id. Ignore any error.

1. Call pq_create with pq_name="mcp-test-pq-lifecycle", heap_size_gb=1, script_body="t = None". Verify success==true. Record the returned id (format: "enterprise:{system_name}:{serial}").

2. Call pq_details with the id from step 1. Verify success==true. Note the current state in the response.

3. Call pq_modify with the id from step 1, setting script_body="t = 42". Verify success==true. Call pq_details again and confirm the config.script_code field changed to "t = 42" (pq_details exposes the script as config.script_code, not script_body).

4. Call pq_start with id set to the id from step 1 (a single ID, not a list). Because the continuous scheduler auto-started this PQ in step 2, pq_start must reject the request. Verify outer success==true AND the single per-item result has success==false with an error mentioning "already RUNNING". If the per-item result has success==true, report FAIL — that indicates the pq_start already-RUNNING guard is broken.

5. Call pq_stop with id set to the id from step 1. Verify success==true. Note that pq_stop returns as soon as the stop request is accepted; the PQ will transition through STOPPING before reaching a terminal stopped state.

6. Poll for the stop to complete: repeatedly call pq_details with the id from step 1 until the returned state is a terminal state (STOPPED, FAILED, KILLED, COMPLETED, or ERROR — i.e., not STOPPING, DISCONNECTED, or any running/initializing state). Use up to 30 attempts with a ~2 second pause between attempts. Verify a terminal state is reached within the polling window; if not, report FAIL with the last observed state and skip step 7.

7. Call pq_restart with id set to the id from step 1. Verify success==true. (Running the poll in step 6 first ensures restart is not racing with an in-flight stop, which can otherwise leave the PQ in STOPPED rather than transitioning back to RUNNING within the tool's default timeout.)

8. CLEANUP: Call pq_delete with id set to the id from step 1. Verify success==true.

9. Call pq_details with the id from step 1. Verify success==false (negative test — deleted PQ must not be found).

Report: PASS/FAIL for each numbered step with a one-line explanation. Include the id from step 1 and the final state observed in step 6.
```
...
```text
Test the enterprise catalog tools. These are read-only operations. The two discovery tools are system-scoped (they take an enterprise system name and need no worker of your own); the two table-level tools are session-scoped (reading a catalog table's schema or rows requires a worker you administer). The catalog may be empty on development systems — steps that require catalog data are conditional. Requires the enterprise system to be ONLINE with its WebClientData persistent query running. Do the following steps in order and report results.
...
3. (Skip if step 2 returned zero tables.) Pick any table from the catalog_tables_list result. Record its namespace and table name separately. Call catalog_table_schema with the SESSION ID from Setup, namespace set to the recorded namespace, and table_name set to the recorded table name. Verify success==true. Note the column_count in the response.

4. (Skip if step 2 returned zero tables.) Call catalog_table_sample with the SESSION ID from Setup, namespace and table_name from step 3, and max_rows=3. Verify success==true. Note the number of rows returned (may be 0 for empty tables).

5. Call catalog_table_schema with the SESSION ID from Setup, namespace="nonexistent_ns_xyz", and table_name="nonexistent_table_xyz". Verify success==false and an error message is returned.
