 ...
 * `toString()` methods) and makes any future change to the marker a
 * one-line edit.
 *
 * Consumers should import {@link REDACTED} rather than hard-coding the
 * literal string, except in test assertions where hard-coding the literal
 * is preferred so the test fails loudly if the canonical value ever changes.
 */

/**
 * Canonical placeholder substituted for any sensitive value in logs,
 * config dumps, or `toString()` output. The bracketed uppercase form is
