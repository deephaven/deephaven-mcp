...
| `system add NAME` | Declares an enterprise system file (`enterprise/systems/<NAME>.json`). Flags: `--url` (connection.json URL), `--auth password\|private_key` plus the matching credential flags (`--username`/`--password`/`--effective-user`; `--key` — the [Deephaven private key](https://deephaven.io/enterprise/docs/sys-admin/admin-glossary/#private-key-login-keypair-files), use `${file:/path/to/priv-<keyname>.base64.txt}`), optional `--max-sessions`, `--heap-gb`. `community` is a reserved name. |
