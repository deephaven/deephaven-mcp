...
- `private_key` → `SessionManager.private_key(key_text)` (the [Deephaven private key](https://deephaven.io/enterprise/docs/sys-admin/admin-glossary/#private-key-login-keypair-files), typically loaded via `${file:/path/to/priv-<keyname>.base64.txt}`).
